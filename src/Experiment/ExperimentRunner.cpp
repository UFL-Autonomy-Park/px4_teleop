...
void ExperimentRunner::init_parameters() {
  ...
  RCLCPP_INFO(node_->get_logger(), "Loaded experiment: %s (%.1f sec @ %.1f Hz)", experiment_id_.c_str(),
              experiment_length_sec_, control_rate_);
}
...
void ExperimentRunner::stop() { /* Some landing procedure here */ }
